...
function AdminLaborModal({ isOpen, onClose }) {
  ...
  const fetchPendingList = async () => {
    try {
      setIsLoading(true);
      // documentApi 연동 (apiFetch가 알아서 result를 반환해줍니다)
      const data = await getPendingLabors(); 
      setPendingList(data || []);
    } catch (error) {
      console.error("대기 목록 조회 실패:", error);
      // apiFetch에서 던진 에러 메시지 활용
  ...
  const handleApprove = async (userId) => {
    ...
    try {
      await approveLaborRole(userId); // documentApi 함수 호출
      alert('승인되었습니다.');
      // 리스트에서 승인된 유저 즉시 제거
  ...
  const handleReject = async (userId) => {
    ...
    try {
      await rejectLaborRole(userId); // documentApi 함수 호출
      alert('거절되었습니다.');
      // 리스트에서 거절된 유저 즉시 제거
      setPendingList(prev => prev.filter(user => user.userId !== userId));
    } catch (error) {
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content-box" style={{ maxWidth: '700px' }} onClick={e => e.stopPropagation()}>
        <div className="modal-header">
          <h2 style={{ color: '#1e293b', margin: 0, fontSize: '20px' }}>👑 노무사 가입 승인 대기 목록</h2>
          <button className="close-btn" onClick={onClose}>✕</button>
        </div>

        <div className="modal-body" style={{ maxHeight: '60vh', overflowY: 'auto', marginTop: '20px' }}>
          {isLoading ? (
            <p style={{ textAlign: 'center', color: '#64748b' }}>목록을 불러오는 중입니다...</p>
          ) : pendingList.length === 0 ? (
            <p style={{ textAlign: 'center', color: '#64748b', padding: '40px 0' }}>대기 중인 노무사 신청이 없습니다.</p>
          ) : (
            <table className="mypage-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead style={{ backgroundColor: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                <tr>
                  <th style={{ padding: '12px', textAlign: 'left', color: '#475569' }}>유저 ID</th>
                  <th style={{ padding: '12px', textAlign: 'left', color: '#475569' }}>이름(닉네임)</th>
                  <th style={{ padding: '12px', textAlign: 'left', color: '#475569' }}>신청 일자</th>
                  <th style={{ padding: '12px', textAlign: 'center', color: '#475569' }}>관리</th>
                </tr>
              </thead>
              <tbody>
                {pendingList.map(item => (
                  <tr key={item.userId} style={{ borderBottom: '1px solid #e2e8f0' }}>
                    ...
                    <td style={{ padding: '12px', textAlign: 'center', display: 'flex', gap: '8px', justifyContent: 'center' }}>
                      {/* 서류 확인 버튼 (item.certificateUrl 사용) */}
                      <button 
